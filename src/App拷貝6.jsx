    ...
    <div className="space-y-8 animate-fade-in relative">
      ...
      <section className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col md:flex-row items-center md:items-start gap-6 relative overflow-hidden">
        ...
        <div className="relative group shrink-0">
          <img src={getUserAvatar(currentUser)} className="w-32 h-32 rounded-full border-4 border-orange-50 shadow-md bg-white object-cover" />
        ...
        <div className="flex-1 w-full z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start w-full">
            ...
            <div className="bg-slate-50 px-5 py-3 rounded-xl border flex items-center gap-3 w-full sm:w-auto">
              <div className="bg-green-100 p-2 rounded-full"><CheckCircle className="w-5 h-5 text-green-600" /></div>
              <div className="text-left">
                <p className="text-xs text-slate-500 font-bold">已完成</p>
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-slate-50 border-b text-slate-500 text-xs uppercase"><th className="px-6 py-4 text-center">排名</th><th className="px-6 py-4">使用者</th><th className="px-6 py-4 text-right">點數</th></tr></thead><tbody className="divide-y divide-slate-100">{users.map((u, i) => (<tr key={i} className="hover:bg-slate-50"><td className="px-6 py-4 text-center font-bold text-slate-500">#{i + 1}</td><td className="px-6 py-4 flex items-center gap-3"><img src={getUserAvatar(u)} className="w-16 h-16 rounded-full bg-white object-cover border" /><span>{u.username}</span></td><td className="px-6 py-4 text-right font-mono font-bold text-orange-600">{u.gPoints}</td></tr>))}</tbody></table></div>
      </div>
    </div>
  );
}
